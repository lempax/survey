...
<div class="row">
    <div class="col-xs-12">
        ...
        <div class="box">
            <div class="box-body">
                <div id="consolidatedContainer" style="height: 400px; width: 100%;"></div>
            </div>
        </div>
        @if(isset($breakdown))
        <div class="box">
...
overview.render();
var consolidated = new CanvasJS.Chart("consolidatedContainer",
        {
        theme: "theme2",
                animationEnabled: true,
                title:{
                text: "{{ $consolidated['name'] . (Request::has('w') ? ' (Week ' . Request::get('w') . ')' : (Request::has('date_start') && Request::has('date_end') ? ' (' . Request::get('date_start') . ' ~ ' . Request::get('date_end') . ')' : ''))}}",
                        fontSize: 30
                },
                toolTip: {
                shared: true
                ...
                axisY: {
                title: "{{ $consolidated['d1Name'] }}"
                },
                axisY2: {
                title: "{{ $consolidated['d2Name'] }}"
                },
                data: [
                {
                type: "column",
                        name: "{{ $consolidated['d1Name'] }}",
                        legendText: "{{ $consolidated['d1Name'] }}",
                        showInLegend: true,
                        dataPoints:[
<?php foreach ($overview['data'] as $i => $data): ?>
                            {label: "<?= $data['label'] ?>", y: <?= $data['data1'] ?>}<?= $i >= (count($overview['data']) - 1) ? '' : ',' ?>
<?php endforeach; ?>
                ...
                {
                type: "column",
                        name: "{{ $consolidated['d2Name'] }}",
                        legendText: "{{ $consolidated['d2Name'] }}",
                        axisYType: "secondary",
                        showInLegend: true,
                        dataPoints:[
<?php foreach ($overview['data'] as $i => $data): ?>
                            {label: "<?= $data['label'] ?>", y: <?= $data['data2'] ?>}<?= $i >= (count($overview['data']) - 1) ? '' : ',' ?>
<?php endforeach; ?>
